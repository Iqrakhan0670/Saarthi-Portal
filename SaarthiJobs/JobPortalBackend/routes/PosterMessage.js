...
const authenticateToken = (req, res, next) => {
  ...
  try {
    ...
    // Normalize the user object - check both role and userType
    req.user = {
      ...decoded,
      userType: decoded.userType || decoded.role, // Add userType from role if not present
      role: decoded.role || decoded.userType       // Add role from userType if not present
    };
    
    console.log("✅ Token authenticated, user:", decoded.email, "ID:", decoded.id, "Type:", req.user.userType);
    next();
  } catch (error) {
