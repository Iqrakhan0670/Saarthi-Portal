...
const apiRequest = async (endpoint, options = {}) => {
  ...
  console.log(
    `[API Request] ${options.method || "GET"} ${API_BASE_URL}${endpoint}, Token: ${token ? "Present" : "Missing"}`,
  );
  try {
    ...
    console.log(
      `[API Response] Status: ${response.status}, Content-Type: ${response.headers.get("Content-Type") || "none"}`,
    );
    if (!response.ok) {
    ...
    if (!contentType.includes("application/json")) {
      ...
      throw new Error(
        `Unexpected response format: Server did not return JSON (Status: ${response.status})`,
      );
    }
...
const Employment = () => {
  ...
  const fetchEmployment = async () => {
    try {
      const response = await apiRequest("/api/useremployments", {
        method: "GET",
      });
      if (response) {
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[fetchEmployment] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
  ...
  const openForm = (index = null) => {
    if (index !== null) {
      ...
      setEmploymentForm({
        ...
        keySkills: employment[index].skills
          ? employment[index].skills.map((skill) => skill.name || skill)
          : [],
      });
    } else {
  ...
  const saveEmployment = async () => {
    console.log(
      "[saveEmployment] Starting save process, form:",
      JSON.stringify(employmentForm, null, 2),
      "editingIndex:",
      editingIndex,
    );
    const errors = [];
    if (!employmentForm.companyName || employmentForm.companyName.trim() === "")
      errors.push("Company Name is required");
    if (!employmentForm.position || employmentForm.position.trim() === "")
      errors.push("Position is required");
    if (
      !employmentForm.startDate ||
      !/^\d{4}-\d{2}-\d{2}$/.test(employmentForm.startDate)
    )
      errors.push("Start Date must be a valid date (YYYY-MM-DD)");
    if (
      employmentForm.endDate &&
      !/^\d{4}-\d{2}-\d{2}$/.test(employmentForm.endDate)
    )
      errors.push("End Date must be a valid date (YYYY-MM-DD)");
    if (
      employmentForm.endDate &&
      employmentForm.startDate &&
      new Date(employmentForm.endDate) < new Date(employmentForm.startDate)
    )
      errors.push("End Date cannot be earlier than Start Date");
    if (
      employmentForm.isOngoing !== undefined &&
      typeof employmentForm.isOngoing !== "boolean"
    )
      errors.push("isOngoing must be a boolean");
    if (
      employmentForm.location &&
      (typeof employmentForm.location !== "string" ||
        employmentForm.location.trim() === "")
    )
      errors.push("Location must be a non-empty string");
    if (
      employmentForm.workDescription &&
      (typeof employmentForm.workDescription !== "string" ||
        employmentForm.workDescription.trim() === "")
    )
      errors.push("Work Description must be a non-empty string");
    if (
      employmentForm.keySkills &&
      (!Array.isArray(employmentForm.keySkills) ||
        employmentForm.keySkills.some(
          (s) => typeof s !== "string" || s.trim() === "",
        ))
    )
      errors.push("Skills must be an array of non-empty strings");
    if (employmentForm.isOngoing && employmentForm.endDate)
      errors.push("End Date must be null if isOngoing is true");
    ...
    try {
      const payload = {
        ...
        endDate: employmentForm.isOngoing
          ? null
          : employmentForm.endDate || null,
        isOngoing: employmentForm.isOngoing || false,
        location: employmentForm.location
          ? employmentForm.location.trim()
          : null,
        workDescription: employmentForm.workDescription
          ? employmentForm.workDescription.trim()
          : null,
      ...
      if (editingIndex !== null && employment[editingIndex]?.id) {
        console.log(
          "[saveEmployment] Updating employment ID:",
          employment[editingIndex].id,
        );
        await apiRequest(
          `/api/useremployments/${employment[editingIndex].id}`,
          {
            method: "PUT",
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[saveEmployment] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else if (error.message.includes("404")) {
        alert(
          "Error: Employment not found or server endpoint issue. Please check the server configuration.",
        );
      } else if (error.message.includes("not JSON")) {
        alert(
          "Error: Server returned an unexpected response (HTML instead of JSON). Please check the server configuration or API URL.",
        );
      } else {
  ...
  const deleteEmployment = async (id, index) => {
    ...
    try {
      ...
      console.log(
        `[deleteEmployment] Successfully deleted employment with ID:`,
        id,
      );
      ...
      if (error.message.includes("404")) {
        ...
        console.log(
          "[deleteEmployment] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else if (error.message.includes("not JSON")) {
        alert(
        ...
        <div className="grid gap-6 mb-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-3">
                    <span>
                      {emp.startDate} -{" "}
                      {emp.isOngoing ? "Present" : emp.endDate}
                    </span>
                    {emp.location && <span>{emp.location}</span>}
                  </div>
                  {emp.workDescription && (
                    <p className="text-gray-600 mb-3">{emp.workDescription}</p>
                  )}
          ...
          {employment.length === 0 && (
            <p className="text-gray-500 text-center w-full py-8">
              No employment added yet. Add your first employment below!
            </p>
          )}
        </div>
