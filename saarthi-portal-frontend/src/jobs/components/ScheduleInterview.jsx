...
const ScheduleInterview = () => {
  ...
  const [selectedInterviewToCancel, setSelectedInterviewToCancel] =
    useState(null);
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("🔐 Token found, API URL:", API_BASE_URL);
      ...
      if (!jobsResponse.ok) {
        ...
        throw new Error(
          `Failed to fetch jobs (${jobsResponse.status}): ${errorData.error || "Unknown error"}`,
        );
      }
      ...
      if (!scheduledResponse.ok) {
        const errorData = await scheduledResponse.json().catch(() => ({}));
        throw new Error(
          `Failed to fetch scheduled interviews (${scheduledResponse.status}): ${errorData.error || "Unknown error"}`,
        );
      }

      const scheduledData = await scheduledResponse.json();
      console.log("✅ Scheduled interviews fetched:", scheduledData.length);
      ...
      for (const job of jobs) {
        ...
        if (appsResponse.ok) {
          ...
          console.warn(
            `Failed to fetch applications for job ${job.id}: ${appsResponse.status}`,
          );
        }
      }
        ...
        if (appStatus === "interview") {
          ...
          deletePromises.push(
            fetch(`${API_BASE_URL}/api/scheduled-interviews/${si.id}`, {
              ...
              console.error(
                `Failed to auto-cancel scheduled interview ${si.id}:`,
                err,
              );
            }),
          );
      ...
      if (deletePromises.length > 0) {
        ...
        console.log(
          `Auto-cancelled ${deletePromises.length} outdated scheduled interviews`,
        );
      }
      ...
      const scheduledAppIds = new Set(
        validScheduledData.map((si) => si.application_id),
      );
      ...
      console.log(
        "Candidates loaded:",
        candidates.map((c) => ({ id: c.id, position: c.position })),
      ); // Debug log
      setError(null);
    } catch (err) {
  ...
  const handleCancelInterview = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/scheduled-interviews/${selectedInterviewToCancel.scheduledInterviewId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    try {
      ...
      if (isEditing) {
        ...
        response = await fetch(
          `${API_BASE_URL}/api/scheduled-interviews/${selectedCandidate.scheduledInterviewId}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await fetch(`${API_BASE_URL}/api/scheduled-interviews`, {
          ...
          body: JSON.stringify({
            application_id:
              selectedCandidate.id || selectedCandidate.applicationId,
            interview_date: interviewDate,
            interview_time: interviewTime,
          }),
        });
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorText || `Failed to schedule interview (HTTP ${response.status})`,
        );
      }
  ...
  useEffect(() => {
    ...
    if (pendingStr) {
      ...
      setSelectedCandidate({
        ...
        avatar:
          pendingApplicant.applicant_name
            ?.split(" ")
            .map((n) => n[0])
            .join("")
            .toUpperCase()
  ...
  const stats = [
    {
      ...
      value: scheduledInterviews.length.toString(),
    },
    {
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="text-lg text-gray-600">
                  Manage your interview pipeline and connect with top candidates
                </p>
              </div>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          {scheduledInterviews.length === 0 ? (
            <p className="text-gray-600 text-center py-8">
              No interviews scheduled yet. Schedule one to get started!
            </p>
          ) : (
          ...
          <div className="relative bg-white rounded-2xl max-w-md w-full shadow-2xl">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-6">
                ...
                {selectedInterviewToCancel?.name} on{" "}
                {formatDate(selectedInterviewToCancel?.scheduledDate)} at{" "}
                {formatTime(selectedInterviewToCancel?.scheduledTime)}?
              </p>
