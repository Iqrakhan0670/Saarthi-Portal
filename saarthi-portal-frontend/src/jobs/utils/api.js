import { getAuthHeaders } from '../../services/apiClient';

// 1. Define the Backend URL
// If you deploy later, you can easily switch this to your live URL
const API_BASE_URL = '/api';

// 2. Use the shared token-aware headers so job seeker and employer requests work with either session key
const getJobApiHeaders = () => getAuthHeaders();

// 3. The Dashboard Stats Function
export const getDashboardStats = async() => {
    try {
        const response = await fetch(`${API_BASE_URL}/admin/dashboard-stats`, {
            method: 'GET',
            headers: getJobApiHeaders(),
        });

        if (!response.ok) {
            throw new Error(`Error: ${response.status} ${response.statusText}`);
        }

        return await response.json();
    } catch (error) {
        console.error("Failed to fetch dashboard stats:", error);
        throw error;
    }
};

// You can add more API functions here later, like:
// export const getCandidates = async () => { ... }