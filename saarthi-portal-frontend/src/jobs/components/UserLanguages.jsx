...
const Languages = () => {
  ...
  const fetchLanguages = async () => {
    try {
      const response = await apiRequest("/api/userlanguages", {
        method: "GET",
      });
  ...
  const addNewForm = (language = null) => {
    if (language !== null) {
      ...
      setForms((prev) => [
        ...prev,
        { languageName: "", proficiency: "Beginner" },
      ]);
    }
  };
  ...
  const saveAllLanguages = async () => {
    ...
    forms.forEach((form, index) => {
      ...
      if (
        ...
        errors.push(
          `Form ${index + 1}: Please select a valid proficiency level.`,
        );
      }
    });
    ...
    try {
      if (editingLanguage !== null) {
        const form = forms[0];
        await apiRequest(`/api/userlanguages/${editingLanguage}`, {
          method: "DELETE",
        });
  ...
  const updateLanguageProficiency = async (languageName, proficiency) => {
    try {
      await apiRequest(`/api/userlanguages/${languageName}`, {
        method: "DELETE",
      });
      await apiRequest("/api/userlanguages", {
        method: "POST",
  ...
  const deleteLanguage = async (languageName) => {
    ...
    try {
      ...
      setLanguages(
        languages.filter((lang) => lang.language_name !== languageName),
      );
    } catch (error) {
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900">
                    {editingLanguage !== null && formIndex === forms.length - 1
                      ? "Edit Language"
                      : "Add Language"}
                  </h2>
                  <button
