...
const apiRequest = async (endpoint, options = {}) => {
  ...
  console.log(
    `[API Request] ${options.method || "GET"} ${endpoint}, Token: ${token ? "Present" : "Missing"}`,
  );
  try {
...
const Accomplishments = () => {
  ...
  const fetchAccomplishments = async () => {
    try {
      ...
      const response = await apiRequest("/api/useraccomplishments", {
        method: "GET",
      });
      ...
      console.log(
        "[fetchAccomplishments] Normalized data:",
        normalizedResponse,
      );
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[fetchAccomplishments] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
  ...
  useEffect(() => {
    console.log(
      "[useEffect] Updating forms based on savedAccomplishments:",
  ...
  const handleInputChange = (category, formIndex, field, value) => {
    console.log(
      `[handleInputChange] Updating ${category}[${formIndex}].${field} to:`,
      value,
    );
    setForms((prev) => ({
      ...prev,
  ...
  const addNewForm = (category, accomplishment = null) => {
    console.log(
      `[addNewForm] Adding form for ${category}, Editing:`,
      !!accomplishment,
      accomplishment,
    );
    if (accomplishment) {
      ...
      if (!id || isNaN(id)) {
        console.error(
          `[addNewForm] Error: Invalid or missing ID for ${category}`,
          accomplishment,
        );
  ...
  const removeForm = (category, formIndex) => {
    console.log(
      `[removeForm] Removing form from ${category} at index:`,
      formIndex,
    );
    setForms((prev) => ({
      ...prev,
  ...
  const saveAllAccomplishments = async () => {
    console.log(
      "[saveAllAccomplishments] Starting save process, forms:",
      forms,
      "editingId:",
      editingId,
    );
    ...
    forms.certifications.forEach((form, index) => {
      ...
      if (!form.issuer || form.issuer.trim() === "")
      ...
      if (
        ...
        errors.push(
          `Certification ${index + 1}: Year must be between 1900 and ${new Date().getFullYear()}.`,
        );
      }
    });
    forms.awards.forEach((form, index) => {
      if (!form.title || form.title.trim() === "")
        errors.push(`Award ${index + 1}: Title is required.`);
      if (!form.issuer || form.issuer.trim() === "")
      ...
      if (
        ...
        errors.push(
          `Award ${index + 1}: Year must be between 1900 and ${new Date().getFullYear()}.`,
        );
      }
    });
    forms.clubs.forEach((form, index) => {
      if (!form.name || form.name.trim() === "")
        errors.push(`Club/Committee ${index + 1}: Name is required.`);
      if (
        form.year &&
        (isNaN(form.year) ||
          form.year < 1900 ||
        ...
        errors.push(
          `Club/Committee ${index + 1}: Year must be between 1900 and ${new Date().getFullYear()}.`,
        );
      }
    });
    ...
    try {
      ...
      for (let form of forms.certifications) {
        console.log(
          "[saveAllAccomplishments] Processing certification:",
          form,
          "editingId.certifications:",
          editingId.certifications,
        );
        if (editingId.certifications && !isNaN(editingId.certifications)) {
          console.log(
            "[saveAllAccomplishments] Updating certification ID:",
            editingId.certifications,
          );
          await apiRequest(
            `/api/useraccomplishments/certifications/${editingId.certifications}`,
            {
              method: "PUT",
              body: JSON.stringify({
                name: form.name.trim(),
          ...
          console.log(
            "[saveAllAccomplishments] Adding new certification:",
            form,
          );
      ...
      for (let form of forms.awards) {
        console.log(
          "[saveAllAccomplishments] Processing award:",
          form,
          "editingId.awards:",
          editingId.awards,
        );
        if (editingId.awards && !isNaN(editingId.awards)) {
          console.log(
            "[saveAllAccomplishments] Updating award ID:",
            editingId.awards,
          );
          await apiRequest(
            `/api/useraccomplishments/awards/${editingId.awards}`,
            {
              method: "PUT",
              body: JSON.stringify({
      ...
      for (let form of forms.clubs) {
        console.log(
          "[saveAllAccomplishments] Processing club:",
          form,
          "editingId.clubs:",
          editingId.clubs,
        );
        if (editingId.clubs && !isNaN(editingId.clubs)) {
          console.log(
            "[saveAllAccomplishments] Updating club ID:",
            editingId.clubs,
          );
          await apiRequest(
            `/api/useraccomplishments/clubs/${editingId.clubs}`,
            {
              method: "PUT",
              body: JSON.stringify({
                name: form.name.trim(),
      ...
      console.log(
        "[saveAllAccomplishments] All requests completed, refetching data...",
      );
      ...
      if (error.message.includes("User profile not found")) {
        ...
        console.log(
          "[saveAllAccomplishments] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
  ...
  const deleteAccomplishment = async (category, id) => {
    ...
    if (!id || isNaN(id)) {
      console.error(
        `[deleteAccomplishment] Error: Invalid or missing ID for ${category}`,
      );
      alert(`Error: Cannot delete ${category.slice(0, -1)}. Invalid ID.`);
      return;
    }
    const confirmed = window.confirm(
      `Are you sure you want to delete this ${category.slice(0, -1)}?`,
    );
    if (!confirmed) {
    ...
    try {
      await apiRequest(`/api/useraccomplishments/${category}/${id}`, {
        method: "DELETE",
      });
      console.log(
        `[deleteAccomplishment] Successfully deleted ${category} with ID:`,
        id,
      );
      ...
      console.error(
        `[deleteAccomplishment] Error deleting ${category.slice(0, -1)}:`,
        error.message,
      );
      if (error.message.includes("404")) {
        alert(
          `Error: ${category.slice(0, -1)} not found. It may have already been deleted.`,
        );
      } else if (
        error.message.includes("401") ||
        error.message.includes("403")
        ...
        console.log(
          "[deleteAccomplishment] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
        ...
        <div className="space-y-8">
          ...
          >
            ...
            <div className="space-y-4">
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          deleteAccomplishment("certifications", cert.id)
                        }
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-lg font-semibold text-gray-800">
                      {editingId.certifications &&
                      !isNaN(editingId.certifications)
                        ? "Edit Certification"
                        : "New Certification"}
                    </span>
                    <button
              ...
              {savedAccomplishments.certifications.length === 0 &&
                forms.certifications.length === 0 && (
          ...
          <div key="awards" className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="space-y-4">
              ...
              {forms.awards.map((form, index) => (
                <div
                  key={`award-form-${index}`}
                  className="p-4 border border-gray-200 rounded-lg"
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-lg font-semibold text-gray-800">
                      {editingId.awards && !isNaN(editingId.awards)
                        ? "Edit Award"
                        : "New Award"}
                    </span>
                    <button
              ...
              {savedAccomplishments.awards.length === 0 &&
                forms.awards.length === 0 && (
                  <p className="text-gray-500 text-center w-full py-8">
            ...
            <div className="space-y-4">
              ...
              {forms.clubs.map((form, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-lg font-semibold text-gray-800">
                      {editingId.clubs && !isNaN(editingId.clubs)
                        ? "Edit Club/Committee"
                        : "New Club/Committee"}
                    </span>
                    <button
              ...
              {savedAccomplishments.clubs.length === 0 &&
                forms.clubs.length === 0 && (
                  <p className="text-gray-500 text-center w-full py-8">
                    No clubs or committees added yet. Add your first club above!
                  </p>
                )}
            </div>
          </div>
