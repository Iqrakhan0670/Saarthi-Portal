...
const Profile = () => {
  ...
  // ─── AI Autofill State ───────────────────────────────────────────────────────
  const [autofillParsed, setAutofillParsed] = useState(null);      // parsed data from Gemini
  const [autofillLoading, setAutofillLoading] = useState(false);   // parsing in progress
  const [autofillSaving, setAutofillSaving] = useState(false);     // saving to DB
  const [autofillFile, setAutofillFile] = useState(null);          // file chosen for parse
  const [autofillSections, setAutofillSections] = useState({       // which sections to apply
    basicInfo: true,
    skills: true,
    education: true,
    experience: true,
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          console.error(
            "[v0] Authentication failed - token may be invalid or expired",
          );
  ...
  /** Step 1: Send resume file to /api/ai/parse-resume → get structured JSON from Gemini */
  const handleParseResume = async () => {
    if (!autofillFile) {
      openAlert("Please choose a resume file to parse.", "error");
      return;
    }
    setAutofillLoading(true);
    setAutofillParsed(null);
    try {
      const token = getToken();
      const form = new FormData();
      form.append("resume", autofillFile);

      const res = await fetch(`${API_BASE_URL}/api/ai/parse-resume`, {
        method: "POST",
      ...
      setAutofillParsed(data.parsed);
      openAlert("✅ Resume parsed! Review the data below and click Apply.", "success");
    } catch (err) {
      console.error("[Autofill] parse error:", err);
      openAlert(`❌ Parsing failed: ${err.message}`, "error");
    } finally {
      setAutofillLoading(false);
    }
  };

  /** Step 2: Send parsed JSON + selected sections to /api/profile/autofill → write to DB */
  const handleApplyAutofill = async () => {
    if (!autofillParsed) return;
    setAutofillSaving(true);
    try {
      const token = getToken();
      const selectedSections = Object.entries(autofillSections)
        .filter(([, checked]) => checked)
        .map(([key]) => key);

      const res = await fetch(`${API_BASE_URL}/api/profile/autofill`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token && { Authorization: `Bearer ${token}` }),
        },
        body: JSON.stringify({ parsed: autofillParsed, sections: selectedSections }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);

      // Step A: refresh DB data first (counts, etc.)
      await fetchUserProfile();
      await fetchCounts();

      // Step B: now apply parsed values ON TOP of the freshly-loaded DB state
      // This ensures resume data always wins over whatever was in the DB.
      setProfileData((prev) => {
        const next = { ...prev };

        if (selectedSections.includes("basicInfo")) {
          const rawName = (autofillParsed.name || "").trim();
          const nameParts = rawName.split(/\s+/);
          const firstName = nameParts[0] || "";
          const lastName = nameParts.slice(1).join(" ") || "";

          // Validate city and state from parsed location
          // NEVER put skill names (React, Tailwind, Redux etc.) into these fields
          const KNOWN_STATES_FE = [
          ...
          const COUNTRY_WORDS_FE = ["india", "usa", "uk", "us", "canada", "australia", "germany", "france"];

          const rawParts = (autofillParsed.location || "").split(",").map(p => p.trim()).filter(Boolean);
          const city = rawParts[0] || "";

          // Only accept state if it's a known state/country — never a skill/framework name
          const rawState = rawParts[1] || "";
          const rawStateLower = rawState.toLowerCase().replace(/[^a-z\s]/g, "").trim();
          const isValidState = rawStateLower &&
            (KNOWN_STATES_FE.some(s => rawStateLower === s || rawStateLower.includes(s)) ||
             COUNTRY_WORDS_FE.includes(rawStateLower));
          const state = isValidState ? rawState : "";

          // Always overwrite city and state with the parsed values (even if empty strings)
          // so fields are cleared if they are not mentioned in the resume
          next.basicInfo = {
            ...next.basicInfo,
            ...(firstName && { firstName }),
            ...(lastName  && { lastName }),
            ...(autofillParsed.email && { email: autofillParsed.email }),
            ...(autofillParsed.phone && { phone: (autofillParsed.phone || "").replace(/\s+/g, " ").trim() }),
            city: city,
            state: state,
          };
          if (autofillParsed.summary) {
            next.profileSummary = autofillParsed.summary;
          }
        }
      ...
      openAlert("🎉 Profile autofilled! Review fields and save.", "success");
      // Clear the autofill state after success
      setAutofillParsed(null);
      setAutofillFile(null);
    } catch (err) {
      console.error("[Autofill] save error:", err);
      openAlert(`❌ Autofill failed: ${err.message}`, "error");
    } finally {
        ...
        setProfileData({
          basicInfo: {
            ...
            lastName:
              userData.last_name && userData.last_name.trim()
                ...
                : "",
            email:
              userData.email && userData.email.trim() ? userData.email : "",
            phone:
              userData.phone && userData.phone.trim() ? userData.phone : "",
            city: userData.city && userData.city.trim() ? userData.city : "",
            state:
              userData.state && userData.state.trim() ? userData.state : "",
            country:
              userData.country && userData.country.trim()
                ? userData.country
                : "",
            preferredLocation:
              userData.preferred_location && userData.preferred_location.trim()
                ? userData.preferred_location
                : "",
            age:
              userData.age && userData.age.toString().trim()
                ? userData.age.toString()
                : "",
            gender:
              userData.gender && userData.gender.trim() ? userData.gender : "",
            jobType:
              userData.job_type && userData.job_type.trim()
                ? userData.job_type
                : "",
            expectedSalary:
              userData.expected_salary &&
              userData.expected_salary.toString().trim()
                ? userData.expected_salary.toString()
                : "",
            profilePhotoUrl:
              userData.profile_photo_url && userData.profile_photo_url.trim()
                ? userData.profile_photo_url
                : "",
          },
          profileSummary:
            userData.profile_summary && userData.profile_summary.trim()
          ...
          languages:
            Array.isArray(userData.languages) && userData.languages.length > 0
              ...
              : [],
          internships:
            Array.isArray(userData.internships) &&
          ...
          accomplishments:
            ...
            Object.keys(userData.accomplishments).length > 0
              ? userData.accomplishments
              : {
                  certifications: [],
                  awards: [],
          ...
          employment:
            Array.isArray(userData.employment) && userData.employment.length > 0
              ? userData.employment
              : [],
          education:
          ...
          resume: {
            file: null,
            url:
              userData.resume_url && userData.resume_url.trim()
                ? userData.resume_url
                : "",
          },
        });
  ...
  const fetchCounts = async () => {
    try {
      ...
      const certCount = Array.isArray(acc.certifications)
        ? acc.certifications.length
        : 0;
  ...
  const calculateCompletion = () => {
    ...
    const sections = {
      ...
      preferredLocation: !!(
        basic.preferredLocation && String(basic.preferredLocation).trim()
      ),
      ...
      expectedSalary: !!(
        basic.expectedSalary && String(basic.expectedSalary).trim()
      ),
      profileSummary: !!(
        profileData.profileSummary && String(profileData.profileSummary).trim()
      ),
      resume: !!(
        profileData.resume?.url && String(profileData.resume.url).trim()
      ),
  ...
  const handleFileUpload = async (event, type) => {
    ...
    const maxSize =
      type === "profilePhoto" ? 5 * 1024 * 1024 : 10 * 1024 * 1024; // 5MB for photos, 10MB for resumes
    ...
    try {
      if (type === "profilePhoto") {
        ...
        reader.onload = (e) => {
          ...
          console.log(
            "[v0] Setting preview photo:",
            dataUrl ? "Data URL created" : "Failed",
          );
        ...
        console.log(
          "[v0] Starting resume upload for file:",
          file.name,
          "size:",
          file.size,
        );
  ...
  const generatePDF = () => {
    ...
    const contactInfo = [];
    if (profileData.basicInfo.email)
      contactInfo.push(`Email: ${profileData.basicInfo.email}`);
    if (profileData.basicInfo.phone)
      contactInfo.push(`Phone: ${profileData.basicInfo.phone}`);
    if (
      profileData.basicInfo.city ||
      profileData.basicInfo.state ||
    ...
    // Save the PDF
    doc.save(
      `${profileData.basicInfo.firstName}_${profileData.basicInfo.lastName}_Resume.pdf`,
    );
  };
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) =>
          !profileData.basicInfo[field] ||
          profileData.basicInfo[field].toString().trim() === "",
      );
  ...
  const getFullResumeUrl = () => {
    if (!profileData.resume.url) return "";
    if (
      profileData.resume.url.startsWith("http://") ||
      profileData.resume.url.startsWith("https://")
    ) {
  ...
  const displayPhotoSrc =
    previewPhoto || (photoUrl ? `${API_BASE_URL}${photoUrl}` : null);
                      ...
                      <img
                        ...
                        onLoad={() => {
                          console.log(
                            ...
                            previewPhoto ? "preview" : "server URL",
                          );
                        }}
                      />
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "lastName", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "city", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "state", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "country", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "age", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleInputChange("basicInfo", "jobType", e.target.value)
                    }
                    className={`w-full p-4 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-gradient-to-r from-blue-50 to-white text-base ${
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center">
                    <span className="text-base text-gray-500 mr-2">
                      {typeof counts.education === "number"
                        ? counts.education
                ...
                >
                  ...
                  <div className="flex items-center">
                    <span className="text-base text-gray-500 mr-2">
                      {typeof counts.languages === "number"
                        ? counts.languages
                  ...
                  <div className="flex items-center">
                    <span className="text-base text-gray-500 mr-2">
                      {counts.accomplishments?.total ??
                        (profileData.accomplishments?.certifications || [])
                          .length +
                          (profileData.accomplishments?.awards || []).length +
                          (profileData.accomplishments?.clubsCommittees || [])
                            .length}{" "}
                      items
                    </span>
                    <svg
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-base font-semibold text-gray-700 mb-2">
                    Upload Resume (PDF, Word .doc/.docx)
                  </label>
            ...
            {/* ── AI Resume Intelligence Autofill Panel ──────────────────── */}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.347.347A5 5 0 0112 17v0a5 5 0 01-3.535-1.465l-.347-.347z" />
                    </svg>
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-white">AI Resume Autofill</h3>
                    <p className="text-violet-200 text-sm">Upload → Gemini parses → Profile fills itself</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Step 1 — Choose your resume (PDF or DOCX)
                  </label>
                  <div className="flex gap-3 items-center">
                    <label className="flex-1 flex items-center gap-3 px-4 py-3 border-2 border-dashed border-violet-300 rounded-xl cursor-pointer hover:border-violet-500 hover:bg-violet-50 transition-all">
                      ...
                      <span className="text-sm text-gray-600 truncate">
                        {autofillFile ? autofillFile.name : "Click to select file…"}
                      </span>
                      <input
                        type="file"
                        accept=".pdf,.docx,.doc"
                    ...
                    <button
                      ...
                      disabled={autofillLoading || !autofillFile}
                      className="shrink-0 flex items-center gap-2 px-5 py-3 bg-violet-600 text-white text-sm font-semibold rounded-xl hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                    >
                      {autofillLoading ? (
                        <>
                ...
                {/* Step 2: Parsed result + section toggles */}
                {autofillParsed && (
                  <div className="space-y-4 animate-fade-in">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        {autofillParsed.name && (
                          <div className="bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Name</span>
                            <p className="text-gray-700 truncate">{autofillParsed.name}</p>
                          </div>
                        )}
                        {autofillParsed.email && (
                          <div className="bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Email</span>
                            <p className="text-gray-700 truncate">{autofillParsed.email}</p>
                          </div>
                        )}
                        {autofillParsed.phone && (
                          <div className="bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Phone</span>
                            <p className="text-gray-700 truncate">{autofillParsed.phone}</p>
                          </div>
                        )}
                        {autofillParsed.location && (
                          <div className="bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Location</span>
                            <p className="text-gray-700 truncate">{autofillParsed.location}</p>
                          </div>
                        )}
                        {(autofillParsed.skills || []).length > 0 && (
                          <div className="col-span-2 bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Skills detected</span>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {autofillParsed.skills.slice(0, 12).map((s, i) => (
                                <span key={i} className="px-2 py-0.5 bg-violet-100 text-violet-700 rounded-full text-xs">{s}</span>
                              ))}
                              {autofillParsed.skills.length > 12 && (
                                <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full text-xs">+{autofillParsed.skills.length - 12} more</span>
                              )}
                            </div>
                          </div>
                        )}
                        {(autofillParsed.education || []).length > 0 && (
                          <div className="col-span-2 bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Education ({autofillParsed.education.length})</span>
                            {autofillParsed.education.slice(0, 2).map((e, i) => (
                              <p key={i} className="text-gray-700 text-xs truncate">{e.degree} — {e.institution}</p>
                            ))}
                          </div>
                        )}
                        {(autofillParsed.experience || []).length > 0 && (
                          <div className="col-span-2 bg-white rounded-lg p-2 border border-violet-100">
                            <span className="text-violet-600 font-medium">Experience ({autofillParsed.experience.length})</span>
                            {autofillParsed.experience.slice(0, 2).map((e, i) => (
                              <p key={i} className="text-gray-700 text-xs truncate">{e.role} @ {e.company}</p>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    {/* Section toggles */}
                    <div>
                      <p className="text-sm font-semibold text-gray-700 mb-2">Step 2 — Select sections to fill:</p>
                      <div className="grid grid-cols-2 gap-2">
                        {Object.entries(autofillSections).map(([key, checked]) => {
                          const labels = {
                            basicInfo: "Basic Info & Summary",
                            skills: "Skills",
                            education: "Education",
                          ...
                          return (
                            <label key={key} className="flex items-center gap-2 cursor-pointer select-none">
                              <input
                                type="checkbox"
                                checked={checked}
                                onChange={() => setAutofillSections(prev => ({ ...prev, [key]: !prev[key] }))}
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-semibold rounded-xl hover:from-violet-700 hover:to-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"
                    >
                      {autofillSaving ? (
                        <>
                    ...
                    <p className="text-xs text-center text-gray-400">
                      ⚠️ This will overwrite existing data in the selected sections.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl shadow-lg p-6 text-white">
              ...
              <p className="text-blue-50 mb-4">
                Create a professional PDF resume from your profile data
              </p>
              <button
                onClick={generatePDF}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-auto">
                {fullResumeUrl &&
                !fullResumeUrl.includes(".doc") &&
                !fullResumeUrl.includes(".docx") ? (
                  <iframe
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-dashed border-blue-300 rounded-lg p-8 text-center h-full flex flex-col items-center justify-center">
                    ...
                    <p className="text-blue-600 text-sm mt-2 mb-6">
