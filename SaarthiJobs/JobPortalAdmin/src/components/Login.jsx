...
// reCAPTCHA Site Key - same across all domains
const SITE_KEY = "6LdJdWUsAAAAAGBtrqEaZ5aOLvZQiF6xZ6M3kaZ9";

export default function Login({ onAuth = () => {} }) {
  const [email, setEmail] = React.useState('')
  const [password, setPassword] = React.useState('')
  const [captchaToken, setCaptchaToken] = React.useState(null) // 3. New State for Token
  ...
  const submit = async (e) => {
    ...
    // 4. Check if Captcha is completed
    if (!captchaToken) {
    ...
    try {
      // 5. Send token along with email/password
