...
const MyJobs = () => {
    ...
    const fetchApplications = async () => {
      try {
        ...
        if (!appRes.ok) {
          ...
          throw new Error(
            `Failed to fetch applications: ${appRes.status} ${appRes.statusText}`,
          );
        }
        ...
        try {
          const lastStatuses = JSON.parse(
            localStorage.getItem("lastAppStatuses") || "{}",
          );
          const changes = mappedJobs.filter(
            (a) =>
              lastStatuses[a.id] &&
              lastStatuses[a.id] !== (a.status || "applied"),
          );
          if (changes.length > 0) {
  ...
  const handleRemoveSavedJob = (jobId) => {
    ...
    window.dispatchEvent(
      new CustomEvent("savedJobsUpdated", { detail: { removed: jobId } }),
    );
  };
                ...
                {savedJobsList.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">
                      Start browsing jobs and save the ones you're interested
                      in!
                    </p>
                  </div>
                )}
                  ...
                  <>
                    ...
                    {appliedJobs.length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">
                          Your application status updates will appear here!
                        </p>
                      </div>
                    )}
                  </>
                  ...
                  <div className="space-y-4">
                    {cvLoadError ? (
                      <div className="text-center py-4 text-red-600">
                        ...
                        <p className="text-sm">
                          Failed to load preview. Use download below.
                        </p>
                      </div>
                    ) : (
                      <iframe
                    ...
                    <button
                      onClick={() =>
                        handleDownloadCV(
                          selectedApp.cv_url,
                          `${selectedApp.applicant_name || "applicant"}_resume.pdf`,
                        )
                      }
      ...
      {showApplicationForm && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-700">
                {editingApplicationId ? "Update Application for" : "Apply for"}{" "}
                ...
                >
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-gray-700">
                      Status changed to {getStatusText(n.status)}
                    </p>
