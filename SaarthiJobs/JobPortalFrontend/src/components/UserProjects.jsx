...
const Projects = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[fetchProjects] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
    ...
    if (index !== null) {
      ...
      setProjectForm({
        ...
        technologies: projects[index].technologies
          ? projects[index].technologies.map((tech) => tech.name || tech)
          : [],
  ...
  const saveProject = async () => {
    console.log(
      "[saveProject] Starting save process, form:",
    ...
    if (
      ...
      errors.push("Technologies must be an array of non-empty strings");
    ...
    try {
      ...
      if (editingIndex !== null && projects[editingIndex]?.id) {
        console.log(
          "[saveProject] Updating project ID:",
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        ...
        alert(
          "Error: Project not found or server endpoint issue. Please check the server configuration.",
        );
  ...
  const deleteProject = async (id, index) => {
    ...
    try {
      ...
      if (error.message.includes("404")) {
        ...
        console.log(
          "[deleteProject] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else if (error.message.includes("not JSON")) {
        alert(
