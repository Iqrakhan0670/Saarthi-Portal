...
const Internships = () => {
  ...
  const fetchInternships = async () => {
    try {
      ...
      const response = await apiRequest("/api/userinternships", {
        method: "GET",
      });
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[fetchInternships] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
  ...
  const saveInternship = async () => {
    console.log(
      "[saveInternship] Starting save process, form:",
      form,
      "editingId:",
      editingId,
    );
    ...
    if (
      form.projectName &&
      (typeof form.projectName !== "string" || form.projectName.trim() === "")
    )
      errors.push("Project Name must be a non-empty string");
    if (
      form.location &&
      (typeof form.location !== "string" || form.location.trim() === "")
    )
    ...
    try {
      ...
      if (error.message.includes("401") || error.message.includes("403")) {
        console.log(
          "[saveInternship] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
  ...
  const deleteInternship = async (id, index) => {
    ...
    try {
      ...
      console.log(
        `[deleteInternship] Successfully deleted internship with ID:`,
        id,
      );
      ...
      if (error.message.includes("404")) {
        ...
        console.log(
          "[deleteInternship] Redirecting to /login due to auth failure",
        );
        navigate("/login");
      } else {
        ...
        <div className="grid gap-6 mb-6">
          {internships.length === 0 ? (
            <p className="text-gray-500 text-center w-full py-8">
              No internships added yet. Add your first internship below!
            </p>
          ) : (
                  ...
                  <div className="flex-1">
                    ...
                    {internship.project_name && (
                      <p className="text-gray-600 mb-2">
                        Project: {internship.project_name}
                      </p>
                    )}
